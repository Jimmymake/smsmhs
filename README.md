...
- `MONGO_URI`
- `SMS_DEFAULT_PROVIDER` (`zeaworld` or `semasms`); it defaults to `zeaworld`
- `ZEAWORLD_USERNAME`, `ZEAWORLD_PASSWORD`, and `ZEAWORLD_SENDER_ID` for Zea World
- `SEMASMS_USERNAME`, `SEMASMS_PASSWORD`, and `SEMASMS_SENDER_ID` for SemaSMS
...
Each send accepts a `provider` field (`zeaworld` or `semasms`), and the selected
